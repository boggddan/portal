...
$( document ).on( 'turbolinks:load', function() {
  ...
  if ( $( '#menu_requirements' ).length ) {
    ...
    if ( sessionStorage.menu_requirements_date_start ) {
      $( '#date_start' ).val( sessionStorage.menu_requirements_date_start ) };
    if ( sessionStorage.menu_requirements_date_end ) {
      $( '#date_end' ).val( sessionStorage.menu_requirements_date_end ) };
    ...
    function filterTableMenuRequirements() {
      ...
      var $path_ajax = $date_start.data( 'ajax-path' ) + '?' + $date_start.attr( 'name' ) + '='
        + $date_start_val + '&' + $date_end.attr( 'name' ) + '=' + $date_end_val;
      $.ajax( { url: $path_ajax, type: 'GET', dataType: 'script' } );
    };
    ...
    deleteMenuRequirement = function() {
      ...
      // Если один одна строка, тогда удаляем всю табличку
      if ( $( 'tbody' ).children().length == 1 ) { $( '#table_menu_requirements' ).empty()
