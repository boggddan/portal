class SyncCatalogsController < ApplicationController
  ...
  def price_product_date( branch, institution, product, date )
    if price_product = PriceProduct.where( branch: branch, institution: institution, product: product ).where( "price_date <= ?", date ).order( :price_date ).last
      price_product
    else
  ...
  def children_day_cost_date( children_category, date )
    if children_day_cost = ChildrenDayCost.where( children_category: children_category ).where( "cost_date <= ?", date ).order( :cost_date ).last
      children_day_cost
    else
  ...
  def institution_order_number( institution, number )
    if institution_order = InstitutionOrder.find_by( institution: institution, number: number )
      ...
      { error: { institution_order: "Не знайдена заявка [#{params[:number]}]" } }
    end
  end
  ...
  def menu_requirement_number( institution, number )
    if menu_requirement = MenuRequirement.find_by( institution: institution, number: number )
      ...
      { error: { menu_requirement: "Не знайдена меню-вимога [#{params[:number]}]" } }
    end
  end
  ...
  def timesheet_number( institution, number )
    if timesheet = Timesheet.find_by( institution: institution, number: number )
      ...
      { error: { timesheet: "Не знайдений табель [#{params[:number]}]" } }
    end
  end
  ...
  def branch_update
    error = { code: "Не знайдений параметр [code]", name: "Не знайдений параметр [name]" }.stringify_keys!.except(*params.keys)
    Branch.create_with( name: params[:name] ).find_or_create_by( code: params[:code] ).update( name: params[:name] )  if error.empty?
    render json: error.any? ? { result: false, error: [error] } : { result: true, code: params[:code] }
  end
  ...
  def branch_view
    ...
    if error.size == 1
      ...
      if error.empty?
        branch = branch_code( params[ :code ] )
        error = branch[ :error ]
      end
    end
  ...
  def institution_update
    error = { code: "Не знайдений параметр [code]", name: "Не знайдений параметр [name]", branch_code: "Не знайдений параметр [branch_code]"  }.stringify_keys!.except(*params.keys)
    if error.empty?
      branch = branch_code( params[:branch_code] )
      unless error = branch[:error]
        update_fields = { code: params[:code], name: params[:name], branch: branch }
        Institution.create_with( update_fields ).find_or_create_by( code: params[:code] ).update( update_fields )
      end
    end

    render json: error && error.any? ? { result: false, error: [error] } : { result: true, code: params[:code] }
  end
  ...
  def institution_view
    ...
    render json: institution ? institution.to_json( include: { branch: {only: [:code, :name] } } ) : { result: false, error: [error] }
  end
  ...
  def product_update
    error = { code: "Не знайдений параметр [code]", name: "Не знайдений параметр [name]" }.stringify_keys!.except(*params.keys)
    Product.create_with( name: params[:name] ).find_or_create_by( code: params[:code] ).update( name: params[:name] )  if error.empty?
    render json: error.any? ? { result: false, error: [error] } : { result: true, code: params[:code] }
  end
  ...
  def product_view
    ...
    if error.size == 1
      ...
      if error.empty?
        product = product_code( params[:code] )
        error = product[:error]
      end
    end
  ...
  def supplier_update
    error = { code: "Не знайдений параметр [code]", name: "Не знайдений параметр [name]" }.stringify_keys!.except(*params.keys)
    Supplier.create_with( name: params[:name] ).find_or_create_by( code: params[:code] ).update( name: params[:name] )  if error.empty?
    render json: error.any? ? { result: false, error: [error] } : { result: true, code: params[:code] }
  end
  ...
  def supplier_view
    ...
    if error.size == 1
      ...
      if error.empty?
        supplier = supplier_code( params[:code] )
  ...
  def children_categories_type_update
    error = { code: "Не знайдений параметр [code]", name: "Не знайдений параметр [name]" }.stringify_keys!.except(*params.keys)
    ChildrenCategoriesType.create_with( name: params[:name] ).find_or_create_by( code: params[:code] ).update( name: params[:name] ) if error.empty?
    render json: error.any? ? { result: false, error: [error] } : { result: true, code: params[:code] }
  end
  ...
  def children_categories_type_view
    ...
    if error.size == 1
      ...
      if error.empty?
        children_categories_type = children_categories_type_code( params[:code] )
  ...
  def children_category_update
    error = { code: "Не знайдений параметр [code]", name: "Не знайдений параметр [name]", children_categories_type_code: "Не знайдений параметр [children_categories_type_code]"  }.stringify_keys!.except(*params.keys)
    if error.empty?
      children_categories_type = children_categories_type_code( params[:children_categories_type_code] )
      unless error = children_categories_type[:error]
        update_fields = { code: params[:code], name: params[:name], children_categories_type: children_categories_type }
        ChildrenCategory.create_with( update_fields ).find_or_create_by( code: params[:code] ).update( update_fields )
      end
    end
  ...
  def children_category_view
    ...
    if error.size == 1
      ...
      if error.empty?
        children_category = children_category_code( params[:code] )
        error = children_category[:error]
      end
    end

    render json: children_category ? children_category.to_json( include: { children_categories_type: {only: [:code, :name] } } ) : { result: false, error: [error] }
  end

  # GET /api/children_categories
  def children_categories_view
    render json: ChildrenCategory.all.order(:code).to_json( include: { children_categories_type: {only: [:code, :name] } } )
  end
  ###############################################################################################
  ...
  def price_product_update
    ...
    if error.empty?
      ...
      PriceProduct.create_with( price: params[:price] ).find_or_create_by!( branch: branch, institution: institution, product: product, price_date: date_int_to_str(params[:price_date]) ).
        update( price: params[:price] ) if error.empty?
    end
  ...
  def price_product_view
    ...
    render json: price_product ? price_product.to_json( include: { branch: { only: [:code, :name] }, institution: { only: [:code, :name] }, product: {only: [:code, :name] } } ) : { result: false, error: [error] }
  end

  # GET /api/price_products
  def price_products_view
    render json: PriceProduct.all.order( :price_date ).to_json( include: { branch: { only: [:code, :name] }, institution: { only: [:code, :name] }, product: {only: [:code, :name] } } )
  end
  ###############################################################################################
  ...
  def children_day_cost_update
    ...
    if error.empty?
      children_category = children_category_code( params[:children_category_code] )

      ChildrenDayCost.create_with( cost: params[:cost] ).find_or_create_by!( children_category: children_category, cost_date: date_int_to_str(params[:cost_date]) ).
  ...
  def children_day_cost_view
    ...
    render json: children_day_cost ? children_day_cost.to_json( include: { children_category: {only: [:code, :name] } } ) : { result: false, error: [error] }
  end

  # GET api/children_day_costs
  def children_day_costs_view
    render json: ChildrenDayCost.all.order( :cost_date ).to_json( include: { children_category: {only: [:code, :name] } } )
  end
  ###############################################################################################
  ...
  def child_update
    error = { code: "Не знайдений параметр [code]", name: "Не знайдений параметр [name]" }.stringify_keys!.except( *params.keys )
    Child.create_with( name: params[ :name ] ).find_or_create_by( code: params[ :code ] ).update( name: params[ :name ] )  if error.empty?
    render json: error.any? ? { result: false, error: [ error ] } : { result: true, code: params[ :code ] }
  end
  ...
  def reasons_absence_update
    error = { code: "Не знайдений параметр [code]", "mark": "Не знайдений параметр [mark]", name: "Не знайдений параметр [name]" }.stringify_keys!.except( *params.keys )
    ReasonsAbsence.create_with( mark: params[ :mark ], name: params[ :name ] ).find_or_create_by( code: params[ :code ] ).update( mark: params[ :mark ], name: params[ :name ] )  if error.empty?
    render json: error.any? ? { result: false, error: [ error ] } : { result: true, code: params[ :code ] }
  end
  ...
  def reasons_absence_view
    ...
    if error.size == 1
      ...
      if error.empty?
        reasons_absence = reasons_absence_code( params[ :code ] )
  ...
  def children_group_update
    error = { code: "Не знайдений параметр [code]", name: "Не знайдений параметр [name]", children_category_code: "Не знайдений параметр [children_category_code]"  }.stringify_keys!.except( *params.keys )
    if error.empty?
      children_category = children_category_code( params[ :children_category_code ] )
      unless error = children_category[ :error ]
        update_fields = { code: params[ :code ], name: params[ :name ], children_category: children_category }
        ChildrenGroup.create_with( update_fields ).find_or_create_by( code: params[ :code ] ).update( update_fields )
      end
    end
  ...
  def children_group_view
    ...
    if error.size == 1
      ...
      if error.empty?
        children_group = children_group_code( params[ :code ] )
        error = children_group[ :error ]
      end
    end

    render json: children_group ? children_group.to_json( include: { children_category: { only: [ :code, :name ] } } ) : { result: false, error: [ error ] }
  end
  ...
  def supplier_order_update
    ...
    if error.empty?
      ...
      if error.empty?
        SupplierOrder.transaction do
          update_fields = { supplier: supplier, date: date_int_to_str(params[:date]), date_start: date_int_to_str(params[:date_start]), date_end: date_int_to_str(params[:date_end]) }

          if supplier_order = branch.supplier_orders.find_by( number: params[:number] )
            supplier_order.update!( update_fields )
          else
            supplier_order = SupplierOrder.create_with( update_fields ).create( branch: branch, supplier: supplier, number: params[:number] )
          end
          ...
          params[:products].each_with_index do |product_par, index|
            ...
            if error.empty?
              ...
              product = product_code( product_par[:product_code] )
              error.merge!( product[:error] ) if product[:error]

              if error.empty?
                supplier_order_products.create_with( count: product_par[:count] ).
                  find_or_create_by!( institution: institution, product: product, contract_number: product_par[:contract_number], date: date_int_to_str(product_par[:date]) ).update( count: product_par[:count] )
              end
            end
    ...
    render json: error && error.any? ? { result: false, error: [error] } : { result: true, number: params[:number] }
  end
  ...
  def supplier_order_view
    ...
    if error.size == 2
      ...
      if error.empty?
        ...
        unless error = branch[:error]
          supplier_order = supplier_order_number( branch, params[:number] )
          error = supplier_order[:error]
        end
      end
    end

    render json: supplier_order ? supplier_order.to_json( include: { branch: { only: [:code, :name] },  supplier: { only: [:code, :name] },
                                                                     supplier_order_products: { include: { institution: { only: [:code, :name] }, product: { only: [:code, :name] } } } } )
  ...
  def receipt_update
    ...
    if error.empty?
      ...
      unless error = institution[:error]
        supplier_order = supplier_order_number( institution.branch, params[:supplier_order_number] )

        unless error = supplier_order[:error]
          Receipt.transaction do
            update_fields = { invoice_number: params[:invoice_number], date: date_int_to_str(params[:date]), date_sa: date_int_to_str(params[:date_sa]), number_sa: params[:number_sa] }

            if receipt = supplier_order.receipts.find_by( institution: institution, contract_number: params[:contract_number], number: params[:number] )
              receipt.update( update_fields )
            else
              receipt = Receipt.create_with( update_fields ).create( supplier_order: supplier_order, institution: institution, contract_number: params[:contract_number] )
            ...
            params[:products].each_with_index do |product_par, index|
              ...
              if error.empty?
                ...
                unless error = product[:error]
                  receipt_products.create_with( count: product_par[:count] ).find_or_create_by!( date: date_int_to_str(product_par[:date]), product: product ).update( count: product_par[:count] )
                end
              end
  ...
  def receipt_view
    ...
    if error.size == 4
      ...
      unless error = institution[:error]
        ...
        unless error = supplier_order[:error]
          receipt = receipt_number( supplier_order, params[:contract_number], params[:number] )
          error = receipt[:error]
        end
      end
    end

    render json: receipt ? receipt.to_json( include: { supplier_order: { only: [:number, :date], include: { branch: { only: [:code, :name] }, supplier: { only: [:code, :name] } } },
  ...
  #    "products": [{"date": "1485296673", "product_code": "000000079  ", "count": 15, "description": "1 тиждень"},
    ...
    if error.empty?
      ...
      unless error = institution[:error]
        InstitutionOrder.transaction do
          update_fields = { date: date_int_to_str(params[:date]), date_start: date_int_to_str(params[:date_start]), date_end: date_int_to_str(params[:date_end]), date_sa: date_int_to_str(params[:date_sa]), number_sa: params[:number_sa] }
          ...
          params[:products].each_with_index do |product_par, index|
            ...
            if error.empty?
              ...
              unless error = product[:error]
                ...
                institution_order_products.create_with( update_fields ).find_or_create_by!( date: date_int_to_str(product_par[:date]), product: product ).update( update_fields )
              end
            end
  ...
  def institution_order_view
    ...
    if error.size == 2
      ...
      if error.empty?
        ...
        unless error = institution[:error]
          institution_order = institution_order_number(institution, params[:number])
          error = institution_order[:error]
        end
      end
    end

    render json: institution_order ? institution_order.to_json( include: { institution: { only: [:code, :name] }, institution_order_products: { include: { product: { only: [:code, :name] } } } } )
      : { result: false, error: [error] }
  end
  ###############################################################################################
  ...
  def io_correction_update
    ...
    if error.empty?
      ...
      unless error = institution[:error]
        institution_order = institution_order_number( institution, params[:institution_order_number] )

        unless error = institution_order[:error]
          IoCorrection.transaction do
            update_fields = { date: date_int_to_str(params[:date]), date_sa: date_int_to_str(params[:date_sa]), number_sa: params[:number_sa] }
            ...
            params[:products].each_with_index do |product_par, index|
              ...
              if error.empty?
                ...
                unless error = product[:error]
                  ...
                  io_correction_products.create_with( update_fields ).find_or_create_by!(date: date_int_to_str(product_par[:date]), product: product).update( update_fields )
                end
              end
  ...
  def io_correction_view
    ...
    if error.size == 3
      ...
      if error.empty?
        ...
        unless error = institution[:error]
          ...
          unless error = institution_order[:error]
            io_correction = io_correction_number(institution_order, params[:number])
    ...
    render json: io_correction ? io_correction.to_json( include: { institution_order: { only: [:number, :date] }, io_correction_products: { include: { product: { only: [:code, :name] } } } } )
      : { result: false, error: [error] }
  end
  ###############################################################################################
    ...
    if error.empty?
      ...
      if error.empty?
        MenuRequirement.transaction do
          ...
          update_fields = { date: date, branch: branch, splendingdate: splendingdate, date_sap: date_int_to_str(params[:date_sap]), number_sap: params[:number_sap] }
          ...
          menu_children_categories.update_all( count_all_plan: 0, count_exemption_plan: 0, count_all_fact: 0, count_exemption_fact: 0 )
          ...
          params[:children_categories].each_with_index do |children_category_par, index|
            error = { children_category_code: "Не знайдений параметр [children_category_code]",
                      ...
                      count_exemption_plan: "Не знайдений параметр [count_exemption_plan]" }.stringify_keys!.except(*children_category_par.keys)

            if error.empty?
              children_category = children_category_code(children_category_par[:children_category_code])

              unless error = children_category[:error]
                update_fields = { count_all_plan: children_category_par[:count_all_plan], count_exemption_plan: children_category_par[:count_exemption_plan], count_all_fact: children_category_par[:count_all_plan], count_exemption_fact: children_category_par[:count_exemption_plan] }
                menu_children_categories.create_with( update_fields ).find_or_create_by!( children_category: children_category ).update( update_fields )
              end
            end
          ...
          params[:products].each_with_index do |product_par, index|
            ...
            if error.empty?
              children_category = children_category_code(product_par[:children_category_code])
              ...
              if error.empty?
                ...
                menu_products.create_with( update_fields ).find_or_create_by!( children_category: children_category, product: product ).update( update_fields )
              end
            end
          ...
          if error.empty?
            menu_children_categories.where( count_all_plan: 0, count_exemption_plan: 0, count_all_fact: 0, count_exemption_fact: 0 ).delete_all
  ...
  def menu_requirement_fact_update
    ...
    if error.empty?
      ...
      if error.empty?
        menu_requirement = menu_requirement_number( institution, params[:number] )
        ...
        unless error = menu_requirement[:error]
          ...
          MenuRequirement.transaction do
            date = date_int_to_str( params[ :date ] )
            splendingdate = params[:splendingdate].empty? ? date : date_int_to_str(params[:splendingdate])
            menu_requirement.update( date: date, branch: branch, splendingdate: splendingdate, date_saf: date_int_to_str(params[:date_saf]), number_saf: params[:number_saf] )
            ...
            params[:children_categories].each_with_index do |children_category_par, index|
              error = { children_category_code: "Не знайдений параметр [children_category_code]",
                        ...
                        count_exemption_fact: "Не знайдений параметр [count_exemption_fact]" }.stringify_keys!.except(*children_category_par.keys)

              if error.empty?
                children_category = children_category_code(children_category_par[:children_category_code])

                unless error = children_category[:error]
                  update_fields = { count_all_fact: children_category_par[:count_all_fact], count_exemption_fact: children_category_par[:count_exemption_fact] }
                  menu_children_categories.create_with( update_fields ).find_or_create_by!( children_category: children_category ).update( update_fields )
                end
              end
  ...
  def menu_requirement_view
    ...
    render json: menu_requirement ? menu_requirement.to_json( include: { branch: { only: [:code, :name] },  institution: { only: [:code, :name] },
      ...
      if error.empty?
        Timesheet.transaction do
          ...
          params[ :dates ].each_with_index do | dates_par, index |
            ...
            if error.empty?
              child = child_code( dates_par[ :child_code ] )
              error.merge!( child[ :error ] ) if child[ :error ]

              children_group = children_group_code( dates_par[ :children_group_code ] )
              error.merge!( children_group[ :error ] ) if children_group[ :error ]

              reasons_absence = reasons_absence_code( dates_par[ :reasons_absence_code ] )
              error.merge!( reasons_absence[ :error ] ) if reasons_absence[ :error ]

              timesheet_dates.create( child: child, children_group: children_group, reasons_absence: reasons_absence, date: date_int_to_str( dates_par[ :date ] ) ) if error.empty?
            end
  ...
  def timesheet_view
    ...
    if error.size == 2
      ...
      if error.empty?
        ...
        unless error = institution[:error]
          timesheet = timesheet_number( institution, params[ :number ] )
          error = timesheet[ :error ]
        end
      end
    end

    render json: timesheet ? timesheet.to_json( include: { branch: { only: [:code, :name] },  institution: { only: [:code, :name] },
                                                           timesheet_dates: { include: { child: { only: [ :code, :name ] },
                                                                                         children_group: { only: [ :code, :name ] },
                                                                                         reasons_absence: { only: [ :code, :mark, :name ] } } } } )
    : { result: false, error: [error] }
  end
  ###############################################################################################
