$( document ).on( 'turbolinks:load', ( ) => {
  ...
  if ( $menuRequirementProducts.length ) {
    ...
    $( '#table_menu_children_categories input' ).change( function() {
      var $this = $( this );
      var $thisOldValue = my.toNumber( $this.data( 'old-value' ), 0 );
      var $thisValue = my.toNumber( $this.val( ), 0 );

      $this.val( my.numToStr( $thisValue, 0 ) );
      ...
      if ( $thisOldValue !== $thisValue ) {
        ...
        $countTotal.html( my.numToStr( my.toNumber( $countTotal.html(), 0 ) - $thisOldValue + $thisValue, 0 ) );
        $this.data( 'ajax-blur',
    ...
    $( '#table_menu_products input' ).change( function() {
      var $this = $( this );
      var $thisOldValue = my.toNumber( $this.data( 'old-value' ), 3 );
      var $thisValue = my.toNumber( $this.val( ), 3 );
      $this.val( my.numToStr( $thisValue, 3) );
      ...
      if ( $thisOldValue != $thisValue ) {
        ...
        switch ( $countType ) {
          case 'diff':
            ...
            $countOldValue = my.toNumber( $count.val(), 3 );
            $countValue = my.toRound($countOldValue - $thisOldValue + $thisValue, 3);1
            $count.val( my.numToStr( $countValue, 3 ) ).data( 'old-value', $countValue );
          ...
          case 'fact':
            ...
            var $countDiffInputValue = my.toNumber( $countDiff.val( ), 3 ) - $thisOldValue + $thisValue;
            $countDiff.val( my.numToStr( $countDiffInputValue, 3 ) )
        ...
        const sumAll = my.toRound( my.toNumber( $countAll.html( ), 3 ) + $countDiffValue, 3);
        //console.log($countAll.html( ), $countDiffValue);
        $countAll.html( my.numToStr( sumAll, 3) );

        var $sumValue = my.toRound( $countValue * my.toNumber( $( '#table_menu_products #price_' + $name_arr[2] ).html( ), 2 ), 2);
        var $sumDiff = $sumValue - my.toNumber( $count.data( 'sum' ), 2);
        $count.data( 'sum', my.numToStr( $sumValue, 2 ) );

        var $childrenCost = $( '#table_menu_children_categories #children_cost_' + $countType + '_' + $name_arr[3] );
        $childrenCost.data( 'sum', my.numToStr( my.toNumber( $childrenCost.data( 'sum' ), 2) + $sumDiff, 2) );
    ...
    function ChildrenCostCalc( $countType, $categoryId ) {
      ...
      var $costValue = my.toNumber( $( '#table_menu_children_categories #cost_' + $categoryId ).html(), 2 );
      var $childrenDiff = $( '#table_menu_children_categories #children_diff_' + $countType + '_' + $categoryId );
      var $countAllValue = my.toNumber( $( '#table_menu_children_categories #count_all_' + $countType + '_' + $categoryId ).val(), 2 );

      var $childrenCostValue = $countAllValue == 0 ? 0 : my.toRound( my.toNumber( $childrenCost.data( 'sum' ), 2 ) / $countAllValue, 2 );
      var $childrenDiffValue = $childrenCostValue - $costValue;

      $childrenCost.html( my.numToStr( $childrenCostValue, 2 ) );
      $childrenDiff.html( my.numToStr( $childrenDiffValue, 2 ) )
